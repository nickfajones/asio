...
public:
   ...
   * follows:
   * @code socket.async_receive_from(
   *     boost::asio::buffer(data, size), 0,
   *     sender_endpoint, destination_endpoint, handler); @endcode
   * See the @ref buffer documentation for information on receiving into
   * multiple buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   */
      ...
      ASIO_MOVE_ARG(ReadHandler) handler)
  {
    ...
    this->get_service().async_receive_from(this->get_implementation(), buffers,
        sender_endpoint, destination_endpoint,
        0, ASIO_MOVE_CAST(ReadHandler)(handler));
  }
